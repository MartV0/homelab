  ...
  outputs = { self, nixpkgs, staticSite, agenix, nixpkgs-unstable }@inputs: let
    ...
    nixosConfigurations = {
      ${hostnamepi} = nixpkgs.lib.nixosSystem rec {
        specialArgs = { inherit flakePath inputs staticSite username agenix; hostname=hostnamepi; system=system_aarch; pkgs-unstable = import nixpkgs-unstable { inherit system; config.allowUnfree = true; };
};
        system = system_aarch;
        modules = [ ./machines/pi4/configuration.nix agenix.nixosModules.default ];
      };
      ${hostnamenuc} = nixpkgs.lib.nixosSystem rec {
        specialArgs = { inherit flakePath inputs staticSite username agenix; hostname=hostnamenuc; system=system_x86; pkgs-unstable = import nixpkgs-unstable { inherit system; config.allowUnfree = true; };
};
        system = system_x86;
